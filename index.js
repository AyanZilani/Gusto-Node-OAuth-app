...
app.get("/company/:realmId", async (req, res) => {
  ...
  request(requestObj, (err, response) => {
    console.log(
      "🚀 ~ file: index.js ~ line 107 ~ request ~ response.body",
      response.body
    );
    ...
    if (err || response.statusCode != 200) {
      return res
        ...
        .json({ error: err, statusCode: response.statusCode });
    }
...
const server = app.listen(process.env.PORT || 4000, function () {
  ...
  if (!ngrok) {
    ...
    console.log(
      "💳  Step 2 : Copy and Paste the clientId and clientSecret from : https://dev.gusto.com"
    );
    console.log(
